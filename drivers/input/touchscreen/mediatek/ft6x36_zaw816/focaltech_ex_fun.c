...
int  fts_5x46_ctpm_Getvender_id(struct i2c_client * client,u8 *ucPVendorID);    //modify by taoxiaodong
...
static unsigned char CTPM_FW1[] = {
	#include "FT5446_AW816_HQ_JL_V14_D02_20150723_app.i"
};   //JL
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
       if  ((Vender_ID==0x79)|| (Vender_ID==0x9b)) // jinlong
	   {
	     pbt_buf = CTPM_FW1;
...
u8 fts_JL_ctpm_get_i_file_ver(void)
{
	u16 ui_sz;
	...
	if (ui_sz > 2)
	{
	    if(fts_updateinfo_curr.CHIP_ID==0x36)
                return CTPM_FW1[0x10a];
	    else
		return CTPM_FW1[ui_sz - 2];
...
u8 fts_ofg_ctpm_get_i_file_ver(void)
{
	u16 ui_sz;
	...
	{
	    if(fts_updateinfo_curr.CHIP_ID==0x36)
                return CTPM_FW2[0x10a];
	    else
		return CTPM_FW2[ui_sz - 2];
...
u8 fts_ctpm_get_i_file_vendor(void)
{
	...
	{
	    if(fts_updateinfo_curr.CHIP_ID==0x36)
                return CTPM_FW1[0x108];
	    else
		return CTPM_FW1[ui_sz - 1];
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	int i_ret;
     ...
     i_ret= fts_5x46_ctpm_Getvender_id(client, &uc_tp_vendor_id);
     if ( i_ret < 0)
     uc_tp_vendor_id = 0xFF;	
     Vender_ID=uc_tp_vendor_id;
...
//	 printk("[FTS] uc_tp_vendor_id = 0x%x, uc_host_vendor_id = 0x%x\n",uc_tp_vendor_id, uc_host_vendor_id);
    // if(uc_tp_vendor_id!=uc_host_vendor_id)return 0;
       ...
       if  ((Vender_ID==0x79)|| (Vender_ID==0x9b))  // jinlong
       {
		uc_host_fm_ver = fts_JL_ctpm_get_i_file_ver();
       }   	
	...
	if (/*the firmware in touch panel maybe corrupted */
		...
		if (i_ret == 0)	{
			...
			if (Vender_ID==0x51)
			 {
				 uc_host_fm_ver = fts_ofg_ctpm_get_i_file_ver();
			 }
			 else if  ((Vender_ID==0x79)|| (Vender_ID==0x9b))  // jinlong
				{
				 uc_host_fm_ver = fts_JL_ctpm_get_i_file_ver();
				}
...
int  fts_5x46_ctpm_Getvender_id(struct i2c_client * client,u8 *ucPVendorID) // add by taoxiaodong 
{
 ...
 int i_ret;
   ...
   fts_read_reg(client, FT_REG_VENDOR_ID, &tp_vid);
   *ucPVendorID =tp_vid;
   if(( tp_vid!=FT_REG_VENDOR_ID)&&( tp_vid !=0))
   return 0;
   
  DBG("[FTS] enter in to bootloader to get Vendor ID ! \n");
  ...
  return -EIO;
   ...
   auc_i2c_write_buf[0] = 0x03;
        ...
        fts_i2c_Write(client, auc_i2c_write_buf, 4);	 /*send param addr*/
         msleep(5);
         reg_val[0] = reg_val[1] = 0x00;
        i_ret = fts_i2c_Read(client, auc_i2c_write_buf, 4, reg_val, 1);

        if (fts_i2c_Read(client, auc_i2c_write_buf, 4, reg_val, 1) < 0) {
          *ucPVendorID = 0;
