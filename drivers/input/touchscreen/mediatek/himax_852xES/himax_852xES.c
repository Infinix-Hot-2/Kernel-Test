...
uint32_t gpDMABuf_pa = NULL;
	...
	static unsigned char i_CTPM_FW_Truly[]=
	{
		//#include "Truly_A31_HuaQin_V02_2015-05-07_1626.i"
	};

	static unsigned char i_CTPM_FW_Shenyue[]=
	{
		#include "TopTouch_CZ501_HuaQin_V09_2015-07-29_1553.i"
	};
#endif
...
void himax_touch_information(void)
{
	...
	hx_print("[himax852xes]:%s, line%d,type=%d\n", __func__, __LINE__, IC_TYPE);
...
static int i_update_FW(void)
{
	...
	int fullFileLength;
	// compiare which vensor is use
	if(private_ts->vendor_sensor_id==0x22){
		ImageBuffer = i_CTPM_FW_Truly;
		fullFileLength = sizeof(i_CTPM_FW_Truly);
		//hx_print("-------zq use Truly FW!--------------\n");
	}else if(private_ts->vendor_sensor_id==0x12)
	{
		ImageBuffer = i_CTPM_FW_Shenyue;
		fullFileLength = sizeof(i_CTPM_FW_Shenyue);

	}else{
		hx_print("HIM TP vendor ID is not match any!\n");
		return 0;	
	}
	if ( private_ts->vendor_config_ver < *(ImageBuffer+(FW_CFG_VER_FLASH_ADDR))
		|| (himax_calculateChecksum(true)==0))
		{
...
static bool himax_ic_package_check(struct himax_ts_data *ts)
{
	...
	if (i2c_himax_read(ts->client, 0x31, data, 3, DEFAULT_RETRY_CNT) < 0)
		return false;
//	hx_print("[himax852xes]:check : data[0]=0x%x, data[1]=0x%x\n",  data[0], data[1]);
//	hx_print("[himax852xes]:check : cmd[0]=0x%x, cmd[1]=0x%x, cmd[2]=0x%x\n", cmd[0], cmd[1],cmd[2]);
		...
		if ( tp_key_index == 0x01) {
			...
			I("APP_switch key pressed\n");
				if (ts->pdata->virtual_key)
				{
					...
					hx_print("-------zq-----close KEY_APP_SWITCH !!!\n");
...
int himax_ts_register_interrupt(struct i2c_client *client)
{
	...
	if(HX_INT_IS_EDGE)
	{
		hx_print("%s edge trigger falling\n ",__func__);
		mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, EINTF_TRIGGER_FALLING, tpd_eint_interrupt_handler, 1);
	}
	else
	{
		hx_print("%s level trigger low\n ",__func__);
...
//				if(HX_INT_IS_EDGE)
				{
#ifdef MTK
					//mt_eint_set_sens(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_EDGE_SENSITIVE);
...
/*				else
				{
#ifdef MTK
					//mt_eint_set_sens(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_LEVEL_SENSITIVE);
					//mt_eint_set_hw_debounce(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_CN);
					mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, EINTF_TRIGGER_LOW, tpd_eint_interrupt_handler, 1);
#endif
#ifdef QCT
...
static int himax852xes_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ts == NULL) {
		printk("%s: allocate himax_ts_data failed\n", __func__);
		err = -ENOMEM;
		goto err_alloc_data_failed;
	}
...
//enable debug of himax
	...
	hx_print("%s: Enable touch down/up debug log since not security-on device",
		__func__);
/*
	if (get_tamper_sf() == 0) {
		ts->debug_log_level |= BIT(3);
...
static void __init himax852xes_init_async(void *unused, async_cookie_t cookie)
{
	...
	if(tpd_driver_add(&tpd_device_driver) < 0)
		hx_print("[himax852xes]:add himax852xes driver failed\n");
...
static int __init himax852xes_init(void)
{
	...
	if (tpd_driver_add(&tpd_device_driver) < 0)
		hx_print("[himax852xes]:add generic driver failed\n");
