...
static int touch_event_handler(void *unused)
{
	...
	static u8 large_detect_status = 0;
   ...
   u8  rqst_data[3] = {(u8)(GTP_REG_RQST >> 8), (u8)(GTP_REG_RQST & 0xFF), 0};
    ...
    do
    {
        ...
        if (DOZE_ENABLED == doze_status)
        {
            ...
            {   
                if (doze_buf[2] == 0xCC){
                    ...
                    input_report_key(tpd->dev, KEY_GESTURE_CC, 1);
                    input_sync(tpd->dev);
                    input_report_key(tpd->dev, KEY_GESTURE_CC, 0);
                    input_sync(tpd->dev);
                    // clear 0x814B
                    doze_buf[2] = 0x00;
        ...
        //add by taoxiaodong  for large_detect
		//large_detect_status = point_data[GTP_ADDR_LENGTH];   
		//if (large_detect_status & 0x40)				
		//{
		//   tpd_up(0,0,0);
		//   large_detect_status =0;
		//   goto exit_work_func;
		//}
		...
		if ((finger & 0x40) == 0x40) 
				{				
						touch_num = 0;
						large_detect_status = 1;			//large bit enable always				
						if(pre_touch)
						{									
					 			tpd_up(0, 0, 0);							
								input_sync(tpd->dev); 
				...
				if(1 == large_detect_status)
				{
						u8 tmp_buf[3] = {0xCE, 0x85};	
						ret = gtp_i2c_read(i2c_client_point, tmp_buf, 3);  //if touch remain,don't exit large touch mode
						GTP_DEBUG("0xCE85 = 0x%02X", tmp_buf[2]);
						if (ret > 0)
						{
								if (0 == tmp_buf[2])
								{
										large_detect_status=0;  //when 0xCE85==0,exit large touch
								}
								else
								{
										touch_num = 0;	  
...
static s8 gtp_wakeup_sleep(struct i2c_client *client)
{
    ...
    while (retry++ < 10)
    {
        ...
        //mt65xx_eint_mask(CUST_EINT_TOUCH_PANEL_NUM); modify by taoxiaodong
          mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);	
        gtp_reset_guitar(client, 20);
        //mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);  modify by taoxiaodong
          mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);		
    #else
...
static void tpd_suspend(struct early_suspend *h)
{
    ...
    mutex_lock(&i2c_access);
#if GTP_GESTURE_WAKEUP
	if((1 == tpd_get_gesture_state())&&(g_call_state != CALL_ACTIVE))
