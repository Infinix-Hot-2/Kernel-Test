...
void primary_display_idlemgr_enter_idle(int need_lock)
{
		...
		disp_helper_get_option(DISP_HELPER_OPTION_IDLEMGR_SWTCH_DECOUPLE))
	{
		...
		__primary_display_switch_mode(DISP_SESSION_DECOUPLE_MODE, pgc->session_id, need_lock);
...
void primary_display_idlemgr_leave_idle(int need_lock)
{
		...
		disp_helper_get_option(DISP_HELPER_OPTION_IDLEMGR_SWTCH_DECOUPLE))
	{
		...
		__primary_display_switch_mode(session_mode_before_enter_idle, pgc->session_id, need_lock);
...
static int _primary_path_idlemgr_monitor_thread(void *data)
{
	...
	{   
		...
		/* before esd lock, path need to unlock */
		//_primary_path_unlock(__func__);
		
		/* sodi will do DL_to_DC, lock is needed to avoid risk (patch is from K_2) */
		//_primary_path_esd_check_lock();

		primary_display_set_idle_stat(1);
		primary_display_idlemgr_enter_idle(0);

		/* sodi is done, unlock esd check */
...
static int _DL_switch_to_DC_fast(void)
{
	...
	//{
	//	spm_enable_sodi(0); /* disable sodi */
	//}
...
static int _DC_switch_to_DL_fast(void)
{
	...
	/* insert disable sodi before switch path */
	//if(primary_display_is_video_mode()==1)
	//{
	//	spm_enable_sodi(0); /* disable sodi */
	//}
...
extern int DSI_LFR_Status_Check();
...
static struct task_struct *primary_display_Recovery_task = NULL;
...
int primary_display_recovery_thread(void)
{
	int ret = 0;
...
unsigned long get_dim_layer_mva_addr(void)
{
    if(dim_layer_mva == 0){
        int frame_buffer_size = ALIGN_TO(DISP_GetScreenWidth(), MTK_FB_ALIGNMENT) * 
                  ALIGN_TO(DISP_GetScreenHeight(), MTK_FB_ALIGNMENT) * 4;
        unsigned long dim_layer_va =  pgc->framebuffer_va + (DISP_GetPages()-1)*frame_buffer_size;
        DISPMSG("init dim layer va_start=0x%lx, va_end=0x%lx,size %d,fb_va start=0x%lx, end=0x%lx\n",
        		dim_layer_va, dim_layer_va+frame_buffer_size, frame_buffer_size, pgc->framebuffer_va, pgc->framebuffer_va+DISP_GetFBRamSize());
        memset(dim_layer_va, 0, frame_buffer_size);
...
int primary_display_init(char *lcm_name, unsigned int lcm_fps)
{
	...
	if(disp_helper_get_stage() == DISP_HELPER_STAGE_NORMAL)
	{
        ...
        primary_display_Recovery_task = kthread_create(primary_display_recovery_thread, NULL, "display_recovery");
...
int primary_display_config_input_multiple(primary_disp_input_config* input)
{
	...
	if(_should_config_ovl_input())
	{
		...
		//}
        //cmdqRecBackupRegisterToSlot(cmdq_handle, pgc->time_info, 0, 0x10008028);
		for(i = 0;i< HW_OVERLAY_COUNT;i++)
		{	
	...
	if(_should_config_ovl_input())
	{
        //cmdqRecBackupRegisterToSlot(cmdq_handle, pgc->time_info, 1,  0x10008028);
		for(i = 0;i< HW_OVERLAY_COUNT;i++)
		{	
	...
	/* backup ovl status for debug */
	if(primary_display_is_video_mode() && !primary_display_is_decouple_mode()) {
			cmdqRecBackupRegisterToSlot(cmdq_handle, pgc->ovl_status_info, 1,  disp_addr_convert(DISP_REG_OVL_STA));
            //cmdqRecBackupRegisterToSlot(cmdq_handle, pgc->time_info, 2,  0x10008028);
	}
...
static UINT32 disp_fb_pages = 3;    //three for framebuffer
...
int primary_display_resolution_test()
{
    int ret=0;
    int i=0;
	unsigned int w_backup   = 0;
