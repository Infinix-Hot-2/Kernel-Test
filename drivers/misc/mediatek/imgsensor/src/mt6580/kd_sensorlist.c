	...
	for(i=0;i<KDIMGSENSOR_MAX_INVOKE_DRIVERS;i++) {
		...
		if(SensorIdx == DUAL_CAMERA_MAIN_SENSOR) {
			for(j=0;j<cntPWROnMain;j++)
				kdCISModulePowerOn( SensorIdx, (char *)g_invokeSensorNameStr[i], false, mode_name);
		} else if(SensorIdx == DUAL_CAMERA_SUB_SENSOR) {
...
MINT32 i = 0;
    ...
    if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
        ...
        /* turn on power */
        ret = _kdCISModulePowerOn((CAMERA_DUAL_CAMERA_SENSOR_ENUM)g_invokeSocketIdx[i], (char *)g_invokeSensorNameStr[i], true, CAMERA_HW_DRVNAME1);
#endif
        if (ERROR_NONE != ret) {
        PK_ERR("[%s]", __func__);
        return ret;
        ...
        if (ERROR_NONE != ret) {
#ifndef CONFIG_FPGA_EARLY_PORTING
        _kdCISModulePowerOn((CAMERA_DUAL_CAMERA_SENSOR_ENUM)g_invokeSocketIdx[i], (char *)g_invokeSensorNameStr[i], false, CAMERA_HW_DRVNAME1);
#endif
        PK_ERR("SensorOpen");
        return ret;
        }
...
/* #ifndef CONFIG_ARM64 */
    ...
    if (1 == pSensorCtrl->on) {
		...
		clkmux_sel(MT_CLKMUX_CAM_MUX_SEL, pSensorCtrl->freq == MCLK_48MHZ_GROUP ?CAM_PLL_48MHZ :CAM_PLL_52MHZ, "CAMERA_SENSOR");
    }
    else {
...
//            mt_set_gpio_mode(GPIO_CAMERA_RCN_A_PIN, GPIO_CAMERA_RDN0_A_PIN_M_CMCSD); /* GPIO 30  */
