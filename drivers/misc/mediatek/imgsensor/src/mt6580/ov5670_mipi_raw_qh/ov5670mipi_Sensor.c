...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV5670MIPI_QH_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static int RG_Ratio_Typical = 0x13B;   // 315
static int BG_Ratio_Typical = 0x123;	//291
...
static int check_otp_AF(int index,int addr)
{
...
static int read_otp_AF(int index, struct otp_struct *otp_ptr)
{
	printk("pxs_ov5670_otp ++++++++++++++++  %s \n", __func__); //pei_add	
		...
		temp = read_cmos_sensor(start_addr);
		(*otp_ptr).infinity_pos = ((temp&0x03)<<8) + read_cmos_sensor(start_addr+1);
		temp = read_cmos_sensor(start_addr+2);
		(*otp_ptr).macro_pos = ((temp&0x03) <<8) + read_cmos_sensor(start_addr+3);
		printk("lili>>>infinity_pos=%d,macro_pos=%d\n",(*otp_ptr).infinity_pos ,(*otp_ptr).macro_pos); 
		sprintf(asus_afotp,"0x%x 0x%x 0x%x 0x%x %03d%03d%02d%02d%02d%02d%02d%02d\n",
								((*otp_ptr).infinity_pos>>2),      // Infinity[9:2]
								((*otp_ptr).infinity_pos&0x03),// Infinity[1:0]
								(*otp_ptr).macro_pos>>2,       //Macro[9:2]
								((*otp_ptr).macro_pos&0x03),    //Macro[1:0]
								read_cmos_sensor(0x705e),
								read_cmos_sensor(0x705f),
								read_cmos_sensor(0x7060),
								read_cmos_sensor(0x7061),
								read_cmos_sensor(0x7062),
								read_cmos_sensor(0x7063),
								read_cmos_sensor(0x7064),
								read_cmos_sensor(0x7065)); 
								
		
		printk("lili>>>module info %d %d %d %d %d %d %d %d\n",read_cmos_sensor(0x705e),
								      read_cmos_sensor(0x705f),
								read_cmos_sensor(0x7060),
								read_cmos_sensor(0x7061),
								read_cmos_sensor(0x7062),
...
static int read_otp_wb(int index, struct otp_struct *otp_ptr)
{
	...
	(*otp_ptr).rg_ratio = (read_cmos_sensor(start_addr+1)<<2) + ((temp>>6) & 0x03);
	(*otp_ptr).bg_ratio = (read_cmos_sensor(start_addr+2)<<2) + ((temp>>4) & 0x03);
	
	temp = read_cmos_sensor(start_addr + 8);
	(*otp_ptr).golden_rg_ratio = (read_cmos_sensor(start_addr+5)<<2) + ((temp>>6) & 0x03);
	(*otp_ptr).golden_bg_ratio = (read_cmos_sensor(start_addr+6)<<2) + ((temp>>4) & 0x03);
...
static int update_otp_wb()
{
	...
	RG_Ratio_Typical = current_otp.golden_rg_ratio;
	BG_Ratio_Typical = current_otp.golden_bg_ratio;
	//calculate G gain
...
printk("lili>>>	rg=%x,bg=%x,  %s \n",rg,bg, __func__); //pei_add
printk("lili>>>>  nR_G_gain=%x, nB_G_gain=%x, nG_G_gain=%x,  %s \n",nR_G_gain,nB_G_gain,nG_G_gain, __func__); //pei_add	
...
printk("lili>>>pxs_ov5670_otp  R_gain=%x, G_gain=%x, B_gain=%x,  %s \n",R_gain ,B_gain,G_gain, __func__); //pei_add	
printk("lili >>>BG_Ratio_Typical = 0x%x ,RG_Ratio_Typical = 0x%x\n",BG_Ratio_Typical,RG_Ratio_Typical);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x5000, 0x57); //huang add 57 5/6
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	mt_set_gpio_mode(GPIO_MAIN_CAM_ID_PIN,GPIO_MAIN_CAM_ID_PIN_M_GPIO);
	mt_set_gpio_dir(GPIO_MAIN_CAM_ID_PIN,GPIO_DIR_IN);
		/***qunhui camera ***/
		LOG_INF("[camera][wangyang]GPIO_MAIN_CAM_ID_PIN：%d \r\n",mt_get_gpio_in(GPIO_MAIN_CAM_ID_PIN));
	if(!mt_get_gpio_in(GPIO_MAIN_CAM_ID_PIN)) //high:qunhui low:huaquan
	{
		LOG_INF("[camera][wangyang]GPIO_MAIN_CAM_ID_PIN：low,is not <huaquan> camera !!!");
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (sensor_id == imgsensor_info.sensor_id) {
				printk("this is ov5670_qunhui module\n");
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
				break;
			}
			LOG_INF("Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
...
UINT32 OV5670_MIPI_RAW_QH_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
