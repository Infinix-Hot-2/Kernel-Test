...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV5670MIPI_HQ_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static int BG_Ratio_Typical = 0x131;   // 305    0.59
static int RG_Ratio_Typical = 0x141;	//321    0.63
...
/*
read af and module info id
*/
static int read_af_info(int index, struct otp_struct *otp_ptr)
{
	printk("pxs_ov5670_otp ++++++++++++++++  %s \n", __func__); //pei_add	
	...
	temp = read_cmos_sensor(end_addr);
	(*otp_ptr).macro_pos = (read_cmos_sensor(start_addr)<<2)+((temp>>6)&0x03);
	(*otp_ptr).infinity_pos = (read_cmos_sensor(start_addr+1)<<2)+((temp>>4)&0x03);
	printk("lili>>>(*otp_ptr).macro_pos=%d  (*otp_ptr).inifity_pos=%d\n",(*otp_ptr).macro_pos,(*otp_ptr).infinity_pos);
	// clear otp buffer
...
static int read_module_info(int index, struct otp_struct *otp_ptr)
{
	printk("pxs_ov5670_otp ++++++++++++++++  %s \n", __func__); //pei_add	
	...
	(*otp_ptr).production_year = read_cmos_sensor(start_addr);
	(*otp_ptr).production_month = read_cmos_sensor(start_addr+1);
	(*otp_ptr).production_day = read_cmos_sensor(start_addr+2);
	(*otp_ptr).serial_number = (read_cmos_sensor(start_addr+3)<<24)
					+(read_cmos_sensor(start_addr+4)<<16)
					+(read_cmos_sensor(start_addr+5)<<8)
					+(read_cmos_sensor(start_addr+6));

	printk("lili>>>module info id=%d %d %d %d \n",(*otp_ptr).production_year,(*otp_ptr).production_month,(*otp_ptr).production_day,(*otp_ptr).serial_number);
	printk("lili>>>>module info 0x%x 0x%x 0x%x 0x%x\n",read_cmos_sensor(start_addr+3),read_cmos_sensor(start_addr+4),
								read_cmos_sensor(start_addr+5),read_cmos_sensor(start_addr+6));

	sprintf(asus_afotp,"0x%x 0x%x 0x%x 0x%x %02d%02d%02d%03d%03d%03d%03d\n",
						...
						read_cmos_sensor(start_addr+3),
						read_cmos_sensor(start_addr+4),
						read_cmos_sensor(start_addr+5),
						read_cmos_sensor(start_addr+6));                  
	// clear otp buffer
	for (i=start_addr; i<=end_addr; i++) {
		write_cmos_sensor(i, 0x00);
...
static int update_otp_AF()
{
	...
	read_af_info(otp_index, &current_otp);
	
	for(i=1;i<=3;i++) {
		temp = check_otp_af(i,0x7040);
	...
	read_module_info(otp_index, &current_otp);
}
#if 0
// index: index of otp group. (1, 2, 3)
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	{
		LOG_INF("[camera][wangyang]GPIO_MAIN_CAM_ID_PIN：high,is not <huaquan> camera !!!");
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C))+1;
			if (sensor_id == imgsensor_info.sensor_id) {
				printk("this is ov5670_huaquan module\n");
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
				break;
			}
			LOG_INF("Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
...
UINT32 OV5670_MIPI_RAW_HQ_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
