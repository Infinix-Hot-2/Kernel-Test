...
#include "stk3x1x.h"
//#include "cust_alsps.c"
#define DRIVER_VERSION          "3.6.0 20150528"
...
#ifdef CALI_EVERY_TIME
		if( (atomic_read(&obj->ps_high_thd_val) > (obj->ps_high_thd_boot + 400)) && (obj->boot_cali == 1) )
		{
			atomic_set(&obj->ps_high_thd_val, obj->ps_high_thd_boot); 
			atomic_set(&obj->ps_low_thd_val, obj->ps_low_thd_boot); 
...
*/
	...
	if(((1<<STK_BIT_PS) &  obj->pending_intr) && (obj->hw->polling_mode_ps == 0))
	{
		...
		if(((flag_reg & STK_FLG_NF_MASK)? 1 : 0) == 0)
		{
			if((obj->ps - obj->psi) > 400) 
			{
				atomic_set(&obj->ps_high_thd_val, obj->psi+300); 
				atomic_set(&obj->ps_low_thd_val, obj->psi+200); 			
		...
		else
		{
			...
			//	if(atomic_read(&obj->ps_high_thd_val) > (obj->ps_low_thd_boot + 400))
			//	{		
			//		atomic_set(&obj->ps_high_thd_val, obj->ps_high_thd_boot); 
			//		atomic_set(&obj->ps_low_thd_val, obj->ps_low_thd_boot); 
			//	}	
...
#ifdef STK_TUNE0
	obj->stk_max_min_diff = obj->hw->diff_val;
	obj->stk_lt_n_ct = obj->hw->lt_ct_val;
	obj->stk_ht_n_ct = obj->hw->ht_ct_val;
#endif
