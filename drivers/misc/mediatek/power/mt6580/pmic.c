...
// LDO Voltage Table
const static int pmic_1v8_voltages[] = {
    1800000,
};

const static int pmic_2v2_voltages[] = {
    2200000,
};

const static int pmic_2v8_voltages[] = {
    2800000,
};

const static int pmic_3v3_voltages[] = {
    3300000,
};

const static int VA_voltages[] = {
    1800000,
    2500000,
};

const static int VCAMA_voltages[] = {
    1500000,
    1800000,
    2500000,
    2800000,    
...
const static int VCN33_voltages[] = {
    3300000,
    3400000,
    3500000,
    3600000,
...
const static int VCN28_voltages[] = {
    1800000,
    2800000,
};
...
const static int VMC_voltages[] = {
    1800000,
    3300000,
};

const static int VMCH_voltages[] = {
    3000000,
    3300000,
};

const static int VEMC3V3_voltages[] = {
    3000000,
    3300000,
};

const static int VGP1_voltages[] = {
    1200000,
    1300000,
    1500000,
    1800000,
...
const static int VGP2_voltages[] = {
    1200000,
    1300000,
    1500000,
    1800000,
...
const static int VGP3_voltages[] = {
    1200000,
    1300000,
    1500000,
    1800000,
...
const static int VCAMAF_voltages[] = {
    1200000,
    1300000,
    1500000,
    1800000,
...
const static int VSIM1_voltages[] = {
    1800000,
    3000000,    
};

const static int VSIM2_voltages[] = {
    1800000,
    3000000,    
};

const static int VIBR_voltages[] = {
    1200000,
    1300000,    
    1500000,
    1800000,    
...
struct mtk_regulator mtk_ldos[] = {
        ...
        PMIC_LDO_GEN1(VCN33_BT,     PMIC_RG_VCN33_EN_BT,    PMIC_RG_VCN33_VOSEL,    VCN33_voltages,         1,PMIC_EN_VOL),
        PMIC_LDO_GEN1(VCN33_WIFI,   PMIC_RG_VCN33_EN_WIFI,  PMIC_RG_VCN33_VOSEL,    VCN33_voltages,         1,PMIC_EN_VOL),     
        ...
        PMIC_LDO_GEN1(VEMC_3V3,     PMIC_RG_VEMC_3V3_EN,    PMIC_RG_VEMC_3V3_VOSEL, VEMC3V3_voltages,       1,PMIC_EN_VOL),
        PMIC_LDO_GEN1(VCAMAF,       PMIC_RG_VCAM_AF_EN,     PMIC_RG_VCAM_AF_VOSEL,  VCAMAF_voltages,        1,PMIC_EN_VOL),     
...
int get_rac_val(void)
{
    ...
    {        
        ...
        if( (curr_2-curr_1) >= 400 ) //40.0mA
        {
            ...
            if(rac_cal<0) ret = rac_cal-(rac_cal*2);
            else          ret = rac_cal;
        }
...
static struct pmic_interrupts interrupts[] = {
    PMIC_M_INTS_GEN(MT6350_INT_STATUS0,MT6350_INT_CON0,MT6350_INT_CON0_SET,MT6350_INT_CON0_CLR,interrupt_status0),
    PMIC_M_INTS_GEN(MT6350_INT_STATUS1,MT6350_INT_CON1,MT6350_INT_CON1_SET,MT6350_INT_CON1_CLR,interrupt_status1),
};
...
void pwrkey_int_handler(void)
{
    ...
    if(pmic_get_register_value(PMIC_PWRKEY_DEB) == 1)
    {
        ...
        {
           kernel_vibrate_flag = false; //vibration flag in MTK_KERNEL_POWER_OFF_CHARGING mode              
           pwrkey_pressed_flag = false;//Powerkey pressed flag for vibration in MTK_KERNEL_POWER_OFF_CHARGING mode
...
// modify by yangbo for chrdet LV
		ret = pmic_config_interface(0x2,0x1,0xF,0x0);  //set chrdet LV=4.2V, default 4.3V
        ...
        ret = pmic_config_interface(0x20,0x1,0x1,2); // [2:2]: RG_USBDL_RST; Maxwell
        ret = pmic_config_interface(0x24,0x1,0x1,1); // [1:1]: RG_BC11_RST; Reset BC11 detection, Tim,zax,charger programming guide
        ret = pmic_config_interface(0x2A,0x0,0x7,4); // [6:4]: RG_CSDAC_STP; align to 6250's setting,Tim, Zax
        ret = pmic_config_interface(0x2E,0x1,0x1,2); // [2:2]: RG_CSDAC_MODE; Tim,Zax
        ret = pmic_config_interface(0x2E,0x0,0x1,6); // [6:6]: RG_HWCV_EN; Align to MT6328,Top off set=1, TIM,Zax
        ret = pmic_config_interface(0x2E,0x1,0x1,7); // [7:7]: RG_ULC_DET_EN; Tim,Zax
        ret = pmic_config_interface(0x40,0x1,0x1,4); // [4:4]: RG_EN_DRVSEL; Ricky
        ret = pmic_config_interface(0x40,0x1,0x1,5); // [5:5]: RG_RST_DRVSEL; Ricky
        ret = pmic_config_interface(0x46,0x1,0x1,1); // [1:1]: PWRBB_DEB_EN; Ricky
        ret = pmic_config_interface(0x48,0x1,0x1,8); // [8:8]: VPROC_PG_H2L_EN; Ricky
        ret = pmic_config_interface(0x48,0x1,0x1,9); // [9:9]: VSYS_PG_H2L_EN; Ricky
        ret = pmic_config_interface(0x4E,0x1,0x1,5); // [5:5]: STRUP_AUXADC_RSTB_SW; need to check with ABB
        ret = pmic_config_interface(0x4E,0x1,0x1,7); // [7:7]: STRUP_AUXADC_RSTB_SEL; need to check with ABB
        ret = pmic_config_interface(0x50,0x1,0x1,0); // [0:0]: STRUP_PWROFF_SEQ_EN; Ricky
        ret = pmic_config_interface(0x50,0x1,0x1,1); // [1:1]: STRUP_PWROFF_PREOFF_EN; Ricky
        ...
        ret = pmic_config_interface(0x102,0x1,0x1,6); // [6:6]: RG_RTC_75K_CK_PDN; JT
        ret = pmic_config_interface(0x102,0x0,0x1,11); // [11:11]: RG_DRV_32K_CK_PDN; JT
        ret = pmic_config_interface(0x102,0x1,0x1,15); // [15:15]: RG_BUCK32K_PDN; JT
        ret = pmic_config_interface(0x108,0x1,0x1,12); // [12:12]: RG_EFUSE_CK_PDN; JT
        ret = pmic_config_interface(0x10E,0x1,0x1,5); // [5:5]: RG_AUXADC_CTL_CK_PDN; JT
        ret = pmic_config_interface(0x120,0x1,0x1,4); // [4:4]: RG_SRCLKEN_HW_MODE; JT
        ret = pmic_config_interface(0x120,0x1,0x1,5); // [5:5]: RG_OSC_HW_MODE; JT
        ret = pmic_config_interface(0x130,0x1,0xFF,8); // [15:8]: RG_TOP_CKTST2_RSV_15_8; JT
        ret = pmic_config_interface(0x148,0x1,0x1,0); // [0:0]: RG_SMT_SYSRSTB; Ricky
        ret = pmic_config_interface(0x148,0x1,0x1,1); // [1:1]: RG_SMT_INT; Ricky
        ret = pmic_config_interface(0x148,0x1,0x1,2); // [2:2]: RG_SMT_SRCLKEN; Ricky
        ret = pmic_config_interface(0x148,0x1,0x1,3); // [3:3]: RG_SMT_RTC_32K1V8; Ricky
        ret = pmic_config_interface(0x160,0x1,0x1,2); // [2:2]: RG_INT_EN_BAT_L; Ricky
        ret = pmic_config_interface(0x160,0x1,0x1,6); // [6:6]: RG_INT_EN_THR_L; Ricky
        ret = pmic_config_interface(0x160,0x1,0x1,7); // [7:7]: RG_INT_EN_THR_H; Ricky
        ret = pmic_config_interface(0x166,0x1,0x1,1); // [1:1]: RG_INT_EN_FCHRKEY; Ricky
        ret = pmic_config_interface(0x212,0x2,0x3,4); // [5:4]: QI_VPROC_VSLEEP; TH
        ret = pmic_config_interface(0x216,0x1,0x1,1); // [1:1]: VPROC_VOSEL_CTRL; Seven,JT
        ret = pmic_config_interface(0x21C,0x17,0x7F,0); // [6:0]: VPROC_SFCHG_FRATE; Seven,JT, the value should setting before EN
        ret = pmic_config_interface(0x21C,0x1,0x1,7); // [7:7]: VPROC_SFCHG_FEN; Seven,JT
        ret = pmic_config_interface(0x21C,0x1,0x1,15); // [15:15]: VPROC_SFCHG_REN; Seven,JT
        ret = pmic_config_interface(0x21E,0x38,0x7F,0); // [6:0]: VPROC_VOSEL; for Low power, the value should after VPROC_VOSEL_CTRL
        ret = pmic_config_interface(0x222,0x18,0x7F,0); // [6:0]: VPROC_VOSEL_SLEEP; Seven, set value same to Vsleep
        ret = pmic_config_interface(0x230,0x3,0x3,0); // [1:0]: VPROC_TRANSTD; Seven,same with 6323
        ret = pmic_config_interface(0x230,0x1,0x3,4); // [5:4]: VPROC_VOSEL_TRANS_EN; Seven,need to recheck
        ret = pmic_config_interface(0x230,0x1,0x1,8); // [8:8]: VPROC_VSLEEP_EN; Seven,JT
        ret = pmic_config_interface(0x238,0x3,0x3,0); // [1:0]: RG_VSYS_SLP; Johnson,same 6323
        ret = pmic_config_interface(0x238,0x2,0x3,4); // [5:4]: QI_VSYS_VSLEEP; Johnson,re-confrim
        ret = pmic_config_interface(0x242,0x23,0x7F,0); // [6:0]: VSYS_SFCHG_FRATE; Johnson,same 6323
        ret = pmic_config_interface(0x242,0x11,0x7F,8); // [14:8]: VSYS_SFCHG_RRATE; Johnson,same 6323
        ret = pmic_config_interface(0x242,0x1,0x1,15); // [15:15]: VSYS_SFCHG_REN; Johnson,same 6323
        ret = pmic_config_interface(0x256,0x3,0x3,0); // [1:0]: VSYS_TRANSTD; Johnson,same 6323
        ret = pmic_config_interface(0x256,0x1,0x3,4); // [5:4]: VSYS_VOSEL_TRANS_EN; Johnson,same 6323
        ret = pmic_config_interface(0x256,0x1,0x1,8); // [8:8]: VSYS_VSLEEP_EN; Johnson,JT
        ret = pmic_config_interface(0x23C,0x1,0x1,1); // [1:1]: VSYS_VOSEL_CTRL; after 0x0256,Johnson
        ret = pmic_config_interface(0x302,0x1,0x3,8); // [9:8]: RG_VPA_CSL; Johnson,same 6323
        ret = pmic_config_interface(0x302,0x1,0x3,14); // [15:14]: RG_VPA_ZX_OS; Johnson,same 6323
        ret = pmic_config_interface(0x310,0x1,0x1,7); // [7:7]: VPA_SFCHG_FEN; Johnson,JT
        ret = pmic_config_interface(0x310,0x1,0x1,15); // [15:15]: VPA_SFCHG_REN; Johnson,JT
        ret = pmic_config_interface(0x326,0x1,0x1,0); // [0:0]: VPA_DLC_MAP_EN; Johnson,same 6323
        ret = pmic_config_interface(0x402,0x1,0x1,0); // [0:0]: VTCXO_LP_SEL; TH,JT
        ret = pmic_config_interface(0x402,0x1,0x1,10); // [10:10]: RG_VTCXO_EN; TH,JT
        ret = pmic_config_interface(0x402,0x0,0x1,11); // [11:11]: VTCXO_ON_CTRL; TH,JT
        ret = pmic_config_interface(0x404,0x1,0x1,0); // [0:0]: VA_LP_SEL; TH,JT
        ret = pmic_config_interface(0x404,0x2,0x3,8); // [9:8]: RG_VA_SENSE_SEL; Check with ricky or KH
        ret = pmic_config_interface(0x500,0x1,0x1,0); // [0:0]: VIO28_LP_SEL; JT,TH
        ret = pmic_config_interface(0x502,0x1,0x1,0); // [0:0]: VUSB_LP_SEL; JT,TH
        ret = pmic_config_interface(0x504,0x1,0x1,0); // [0:0]: VMC_LP_SEL; JT,TH
        ret = pmic_config_interface(0x506,0x1,0x1,0); // [0:0]: VMCH_LP_SEL; JT,TH
        ret = pmic_config_interface(0x508,0x1,0x1,0); // [0:0]: VEMC_3V3_LP_SEL; JT,TH
        ret = pmic_config_interface(0x514,0x1,0x1,0); // [0:0]: RG_STB_SIM1_SIO; CC
        ret = pmic_config_interface(0x516,0x1,0x1,0); // [0:0]: VSIM1_LP_SEL; Chek with Lower power team
        ret = pmic_config_interface(0x518,0x1,0x1,0); // [0:0]: VSIM2_LP_SEL; Chek with Lower power team
        ret = pmic_config_interface(0x524,0x1,0x1,0); // [0:0]: RG_STB_SIM2_SIO; CC
        ret = pmic_config_interface(0x550,0x1,0x1,1); // [1:1]: VRF18_ON_CTRL; JT,TH
        ret = pmic_config_interface(0x552,0x1,0x1,0); // [0:0]: VM_LP_SEL; JT,TH
        ret = pmic_config_interface(0x556,0x1,0x1,0); // [0:0]: VIO18_LP_SEL; JT,TH
        ret = pmic_config_interface(0x756,0x1,0x3,2); // [3:2]: RG_ADC_TRIM_CH6_SEL; Ricky
        ret = pmic_config_interface(0x756,0x1,0x3,4); // [5:4]: RG_ADC_TRIM_CH5_SEL; Ricky
        ret = pmic_config_interface(0x756,0x1,0x3,8); // [9:8]: RG_ADC_TRIM_CH3_SEL; Ricky
        ret = pmic_config_interface(0x756,0x1,0x3,10); // [11:10]: RG_ADC_TRIM_CH2_SEL; Ricky
        ret = pmic_config_interface(0x778,0x1,0x1,15); // [15:15]: RG_VREF18_ENB_MD; JT confirmed with ZF


        /* GPIO5 WORKAROUND */
        ret = pmic_config_interface(0x416,0x1,0x1, 7); // [7:7]: RG_VCN33_EN_BT           
        ret = pmic_config_interface(0x418,0x1,0x1, 12); // [12:12]: RG_VCN33_EN_WIFI
