...
//CHR_CURRENT_ENUM USB_CHARGER_CURRENT1[4]={CHARGE_CURRENT_300_00_MA,CHARGE_CURRENT_550_00_MA,CHARGE_CURRENT_300_00_MA,CHARGE_CURRENT_300_00_MA};
//CHR_CURRENT_ENUM AC_CHARGER_CURRENT1[4]={CHARGE_CURRENT_850_00_MA,CHARGE_CURRENT_850_00_MA,CHARGE_CURRENT_850_00_MA,CHARGE_CURRENT_850_00_MA};
BATTERY_VOLTAGE_ENUM TEM_VOL[3]={BATTERY_VOLT_04_340000_V,BATTERY_VOLT_04_340000_V,BATTERY_VOLT_04_340000_V};
BATTERY_VOLTAGE_ENUM TEM_VOL2[3]={BATTERY_VOLT_04_100000_V,BATTERY_VOLT_04_100000_V,BATTERY_VOLT_04_100000_V};
BATTERY_VOLTAGE_ENUM g_cv_voltage;
#endif
...
//modify by gyg 2015.3.9
#if defined(BATTERY_TEMP_CHARGING_CURRENT)

E_BAT_TEMP g_tempStatus = E_BAT_T_B_10_48;

void check_battery_temperature(void)
{
   printk("charger temperature: %d\n",BMT_status.temperature);

  	if(BMT_status.temperature < MIN_CHARGE_TEMPERATURE) {
               g_tempStatus = E_BAT_T_L_0;
       }
       else if((BMT_status.temperature >= MIN_CHARGE_TEMPERATURE )&&(BMT_status.temperature< CHARGE_TMEPERATURE_10)) {
               g_tempStatus = E_BAT_T_B_0_10;
       }
       else if((BMT_status.temperature >= CHARGE_TMEPERATURE_10)&&(BMT_status.temperature < CHARGE_TMEPERATURE_48)) {
               g_tempStatus = E_BAT_T_B_10_48;
       }
       else if((BMT_status.temperature >= CHARGE_TMEPERATURE_48)&&( BMT_status.temperature < MAX_CHARGE_TEMPERATURE)) {
               g_tempStatus = E_BAT_T_B_48_55;
       }
       else if(BMT_status.temperature >= MAX_CHARGE_TEMPERATURE) {
               g_tempStatus = E_BAT_T_U_55;
       }
...
void set_vol_according_to_temperature(void)
{
	check_battery_temperature();
		...
		switch(g_tempStatus)
               {
		      case E_BAT_T_L_0:
                                   	  g_cv_voltage = TEM_VOL[g_fg_battery_id];
                               break;
                      case E_BAT_T_B_0_10:
                                          g_cv_voltage = TEM_VOL[g_fg_battery_id];
                               break;
                      case E_BAT_T_B_10_48:
                                          g_cv_voltage = TEM_VOL[g_fg_battery_id];
                               break;
                      case E_BAT_T_B_48_55:
                                  	 g_cv_voltage = TEM_VOL2[g_fg_battery_id];
                               break;
		      case E_BAT_T_U_55:
                                   	 g_cv_voltage = TEM_VOL2[g_fg_battery_id];
                               break;
		...
		#else
		     g_cv_voltage = BATTERY_VOLT_04_340000_V;
		#endif
	#else
	         g_cv_voltage = BATTERY_VOLT_04_200000_V;
	#endif 
#endif 
}
...
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
		      case E_BAT_T_L_0:
                                   	  g_temp_input_CC_value = AC_CHARGER_CURRENT_00C;
                                   	  g_temp_CC_value = AC_CHARGER_CURRENT_00C;
                               break;
                      case E_BAT_T_B_0_10:
...
static kal_uint32 charging_full_check(void)
{
 ...
 #if defined(BATTERY_TEMP_CHARGING_CURRENT)
     if(( status == KAL_TRUE)||((BMT_status.bat_temp_status == E_BAT_T_B_48_55)&&(BMT_status.bat_vol > BATTERY_VOLT_04_100000_V / 1000)))
 #else
         if ( status == KAL_TRUE) 
 #endif
	 {
...
//gyg_add_20150612 add
extern int g_runin_test_enter;
//gyg for battery runin
void pchr_turn_on_charging(void)
{
#if !defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
	BATTERY_VOLTAGE_ENUM cv_voltage;
...
#if !defined(HQ_CUSTOM_DISABLE_CURRENT_BCCT)
	...
	#ifdef HIGH_BATTERY_VOLTAGE_SUPPORT
		#if defined(BATTERY_TEMP_CHARGING_CURRENT)
		    	set_vol_according_to_temperature();
			battery_log(BAT_LOG_CRTI, "set_vol_according_to_temperature**g_cv_voltage[%d]=%d***gyg--BATTERY_TEMP_CHARGING_CURRENT_ok\n",g_fg_battery_id,g_cv_voltage);
  		#else		
			g_cv_voltage = BATTERY_VOLT_04_340000_V;
		#endif
	#else
