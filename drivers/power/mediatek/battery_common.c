...
#define PWRKEY_LONG_PRESS_TIME          (500*1000000)    //500ms+       
extern BOOL pwrkey_pressed_flag;
extern BOOL kernel_vibrate_flag;
static unsigned long timer_init = 0;
static unsigned long timer_end = 0; 
...
//add by yangbo for chrdet retry 3 times
...
//add by yangbo for chrdet retry 3 times
	...
	printk(KERN_ERR "[upmu_is_chr_det][Bowen] ret = %d ;retry = %d vchr = %d!\n",ret,retry,vchr);
	}while((ret == KAL_FALSE)&&(retry>=0));
	return ret;
#endif
}
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BatteryAverageCurrent:
		//gyg  g_boot_mode_ex==10 number to factory mode ; g_boot_mode_ex==11 combination key to factory mode
		// the current tested by the factory is not the actural current
		//this leads to unable to distinguish the healthy of battery

		    val->intval  = battery_meter_get_charging_current();
...
static kal_bool mt_battery_100Percent_tracking_check(void)
{
	...
	static kal_uint32 timer_counter;// = (batt_cust_data.onehundred_percent_tracking_time / BAT_TASK_PERIOD);
	static int timer_counter_init = 0;
#endif
...
static kal_bool mt_battery_nPercent_tracking_check(void)
{
	...
	if ((BMT_status.ZCV <= BMT_status.nPercent_ZCV)
		...
		if (timer_counter == (batt_cust_data.npercent_tracking_time / BAT_TASK_PERIOD))	/* every x sec decrease UI percentage */
		{
			BMT_status.UI_SOC--;
			timer_counter = 1;
		} else {
...
static void battery_update(struct battery_data *bat_data)
{
	...
	bat_data->battery_id=g_fg_battery_id;
	battery_log(BAT_LOG_CRTI,"[battery_update]gyg-Battery id=(%d),bat_data->battery_id=(%d)\n",g_fg_battery_id,bat_data->battery_id);
#else
	bat_data->battery_id=4;
	battery_log(BAT_LOG_CRTI,"[battery_update]gyg-Battery id=(%d)\n",bat_data->battery_id);
#endif
	...
	{	if (BMT_status.bat_exist) {	/* charging */
		...
		battery_log(BAT_LOG_CRTI,"[battery_update]gyg-bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_NOT_CHARGING\n");
		if (BMT_status.bat_vol <= batt_cust_data.v_0percent_tracking)
			resetBatteryMeter = mt_battery_0Percent_tracking_check();
		else
...
PMU_STATUS do_batt_temp_state_machine(void)
{
	...
	if (batt_cust_data.bat_low_temp_protect_enable)
	{
		...
		battery_log(BAT_LOG_CRTI,"[do_batt_temp_state_machine]gyg-temperature<%d\n",batt_cust_data.min_charge_temperature);
...
static PMU_STATUS mt_battery_CheckBatteryTemp(void)
{
	...
	if (do_batt_temp_state_machine() == PMU_STATUS_FAIL) {
		...
		battery_log(BAT_LOG_CRTI,"[mt_battery_CheckBatteryTemp]gyg-do_batt_temp_state_machine() == PMU_STATUS_FAIL\n");
		status = PMU_STATUS_FAIL;
	}
	}
	else
...
static void mt_battery_CheckBatteryStatus(void)
{
	...
	if (mt_battery_CheckBatteryTemp() != PMU_STATUS_OK) {
		BMT_status.bat_charging_state = CHR_ERROR;
		battery_log(BAT_LOG_CRTI,"[mt_battery_CheckBatteryStatus]gyg-BMT_status.bat_charging_state = CHR_ERROR\n");
		return;
	}
...
#if 1//taoxiaodong 150623 add for identify the flash ID
extern unsigned int g_emmc_raw_cid[4]; /* raw card CID */
extern void msdc_emmc_id_check();
#endif
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	int adc_out_data[5] = { 1, 1,1,1,1 };
	 #if 1//huangshiguo 130529 add for identify the flash ID
	 char temp_buf[10];
	 #endif
    ...
    //
    #ifdef HQ_CHARGER_FOR_HUAWEI    
    kal_uint32 icharging_enable = KAL_FALSE;
    #endif	
...
//erick add begin
static ssize_t show_current_state(struct device *dev, struct device_attribute *attr, char *buf)
{
	unsigned int temp_state;

	temp_state = charging_get_current_state();
	battery_log(BAT_LOG_CRTI, "[Battery]  g_current_state: %d\n", temp_state);
	
	return sprintf(buf, "%u\n", temp_state);
}

static DEVICE_ATTR(current_state, 0664, show_current_state, NULL);
//erick add end
#endif
/* ///////////////////////////////////////////////////////////////////////////////////////// */
/* // platform_driver API */
...
#if defined(CONFIG_CHARGING_STATE_NODE)
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_current_state);//erick add
#endif
