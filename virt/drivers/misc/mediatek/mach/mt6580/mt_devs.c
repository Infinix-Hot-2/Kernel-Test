...
//FIXME add this for walk around build error for Rainier FPGA Bring up 
void  __attribute__((weak)) adjust_kernel_cmd_line_setting_for_console (char *u_boot_cmd_line, char *kernel_cmd_line)
{
 return;
}
...
static struct resource mtk_resource_nand[] = {
	...
	{
		.start		= NFI_IRQ_BIT_ID,
		.flags		= IORESOURCE_IRQ,
	},
	{
		.start		= NFIECC_IRQ_BIT_ID,
		.flags		= IORESOURCE_IRQ,
	},
};
...
void mt_dt_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
    ...
    printk(KERN_ALERT"mt_dt_fixup()-->%d ---- \n" , g_boot_mode);   //songjiangchao,2015-03-31,add
...
//      parse_tag_videofb_fixup(tags);
    ...
    else if((g_boot_mode == UNKNOWN_BOOT) || (g_boot_mode == FACTORY_BOOT)){
	    if(tags->u.boot.bootmode == 11) //APK Test mode startup
	    {
