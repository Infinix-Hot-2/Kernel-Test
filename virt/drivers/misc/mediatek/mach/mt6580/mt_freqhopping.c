...
static const UINT32 pllid_to_hp_con[] = {0, 1, 3, 4};
...
static void mt_fh_hal_default_conf(void)
{
    FH_MSG_DEBUG("%s",__func__);
       ...
       // freqhopping_config(FH_MEM_PLLID, g_default_freq[FH_MEM_PLLID], true);
        ...
        fh_set_field(REG_FHCTL_DSSC_CFG, (0x1U<<3), 0);  ///< Set FHCTL_DSSC_CFG(0x1000CF14), Bit3 is RF3(LTE) SSC control. Clear to 0 is enable.
	...
	//FH_MSG("5. write back to ncpo");
    {
        ...
        if(pll_id == FH_MEM_PLLID)
        {
            ...
            fh_write32(reg_pll_con1,
            	       ((fh_read32(g_reg_mon[pll_id])&MASK21b) << 11)   ///< left shift 10bit to [30:20]
...
static int fh_dumpregs_proc_read(struct seq_file* m, void* v)
{
	...
	seq_printf(m, "ARMCA7:0x%08x M:0x%08x MAIN:0x%08x MEM:0x%08x ",
			DRV_Reg32(REG_ARMCA7PLL_CON0),	DRV_Reg32(REG_MAINPLL_CON0),
			DRV_Reg32(REG_WHPLL_CON0),	DRV_Reg32(REG_MEMPLL_CON0));

	seq_printf(m, "\r\nPLL_CON1 :\r\n");
	seq_printf(m, "ARMCA7:0x%08x M:0x%08x MAIN:0x%08x MEM:0x%08x ",
			DRV_Reg32(REG_ARMCA7PLL_CON1),	DRV_Reg32(REG_MAINPLL_CON1),
			DRV_Reg32(REG_WHPLL_CON1),	DRV_Reg32(REG_MEMPLL_CON1));
...
static int __fh_debug_proc_read(struct seq_file* m, void* v, fh_pll_t * pll)
{
	...
	seq_printf(m, "===============================================\r\n" );
	seq_printf(m, "id=ARMCA7PLL=MAINPLL=WHPLL=MEMPLL\r\n" );
