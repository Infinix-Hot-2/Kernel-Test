...
//Disable ACCDET
		...
		mutex_unlock(&accdet_eint_irq_sync_mutex);
		#ifdef CONFIG_DTV
			ACCDET_DEBUG("[Accdet] defined CONFIG_DTV-gyg ok\n");
...
static irqreturn_t accdet_eint_func(int irq,void *data)
{
	...
	*/
		if (CUST_EINT_ACCDET_POLARITY == CUST_EINT_POLARITY_HIGH){
...
#else
	mt_eint_set_hw_debounce(CUST_EINT_ACCDET_NUM, CUST_EINT_ACCDET_DEBOUNCE_CN);
	mt_eint_registration(CUST_EINT_ACCDET_NUM, CUST_EINT_ACCDET_POLARITY, accdet_eint_func, 0);
