...
//				DISPMSG("[callback]%s  EXT  te\n", ddp_get_module_name(module));
		...
		{
			//MMProfileLogEx(ddp_mmp_get_events()->dsi_frame_done, MMProfileFlagPulse, DSI_REG[0]->DSI_LFR_STA.LFR_SKIP_CNT, DSI_REG[0]->DSI_LFR_STA.LFR_SKIP_STA);
		}
	}
}
...
DSI_STATUS DSI_TXRX_Control(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	bool hstx_cklp_en = TRUE;    //dsi_params->cont_clock?FALSE:TRUE;    //modify by taoxiaodong
...
//#if 0
            if (0 != DSI_REG[d]->DSI_MODE_CTRL.MODE){//not in cmd mode
                ...
                //start DSI VM CMDQ
                if(force_update){
                    MMProfileLogEx(MTKFB_MMP_Events.DSICmd, MMProfileFlagStart, *(unsigned int*)(&DSI_VM_CMD_REG[d]->data[0]), *(unsigned int*)(&DSI_VM_CMD_REG[d]->data[1]));
                    DSI_EnableVM_CMD(module, cmdq);

                    //must wait VM CMD done?
                    MMProfileLogEx(MTKFB_MMP_Events.DSICmd, MMProfileFlagEnd, *(unsigned int*)(&DSI_VM_CMD_REG[d]->data[2]), *(unsigned int*)(&DSI_VM_CMD_REG[d]->data[3]));
                }
            }
            else
//#endif
...
//	static cmdqBackupSlotHandle hSlot = 0;
			DSI_RX_DATA_REG read_data1;   //modify by taoxiaodong
			DSI_RX_DATA_REG read_data2;
			DSI_RX_DATA_REG read_data3;
	...
	{	
		...
		for(i = 0; i < 3; i++)
		{
			...
			t0.Data1   = 0;	
			t1.CONFG = 0x00;    //modify by taoxiaodong
			...
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[0], AS_UINT32(&t1));  //modify by taoxiaodong
		   ...
		   //     DSI_OUTREGBIT(cmdq_trigger_handle, DSI_INT_STATUS_REG, DSI_REG[dsi_i]->DSI_INTSTA, RD_RDY, 0x00000001);
	       // }
			// 1. wait DSI RD_RDY(must clear, in case of cpu RD_RDY interrupt handler)    //modify by taoxiaodong
			if(dsi_i==0) // DSI0
			{
					ret = cmdqRecPoll(cmdq_trigger_handle, 0x1401200c, 0x1, 0x00000001);
					ret = cmdqRecWrite(cmdq_trigger_handle, 0x1401200c, 0x0, 0x00000001);
			}
	       ...
	       // }
			// 2. save RX data     //modify by taoxiaodong
			if(hSlot)
			{
					cmdqRecBackupRegisterToSlot(cmdq_trigger_handle, hSlot[0], i, 0x14012074);
					cmdqRecBackupRegisterToSlot(cmdq_trigger_handle, hSlot[1], i, 0x14012078);
					cmdqRecBackupRegisterToSlot(cmdq_trigger_handle, hSlot[2], i, 0x1401207C);
					cmdqRecBackupRegisterToSlot(cmdq_trigger_handle, hSlot[3], i, 0x14012080);
		...
		for(i = 0; i < 3; i++)
		{
			...
			if(hSlot)
			{
				...
				//cmdqBackupReadSlot(hSlot, i, &read_data0);
                  cmdqBackupReadSlot(hSlot[0], i, &read_data0);   //modify by taoxiaodong 
                  cmdqBackupReadSlot(hSlot[1], i, &read_data1);
                  cmdqBackupReadSlot(hSlot[2], i, &read_data2);
				  cmdqBackupReadSlot(hSlot[3], i, &read_data3); 	
			}
			else
			{
			...
			DISPDBG("[DSI]enter cmp DSI+0x0c=0x%x\n",AS_UINT32(DDP_REG_BASE_DSI0+0x0c));
              ...
              DISPCHECK("[DSI]enter cmp read_data0 byte0=0x%x byte1=0x%x byte2=0x%x byte3=0x%x \n",read_data0.byte0,read_data0.byte1,read_data0.byte2,read_data0.byte3);          //modify by taoxiaodong
			...
			packet_type = read_data0.byte0;       //modify by taoxiaodong
			DISPCHECK("DSI read packet_type is 0x%x \n",packet_type);
