...
static int ps_open_status=false;
static int ps_privours_value=5;
...
static void als_work_func(struct work_struct *work)
{
	...
	/**********************************************************************************/
	if(ps_open_status==true){
	err = cxt->ps_data.get_data(&value,&status);
	
	ALSPS_ERR("ps_work_func--get ps data is OK!!\n" );
	if(err)
	{
		ALSPS_ERR("get alsps data fails!!\n" );
		goto ps_loop;
	...
	else
	{
    	...
    	if(!(ps_privours_value==value)){
	ps_privours_value=value;
	...
	ALSPS_LOG(" ps_data_report is Ok \n");
...
{	
	...
	if(enable == 1){
		if(alsps_context_obj->is_als_active_data == false)
			...
			ret = cxt->ps_ctl.enable_nodata(enable);
	}else if(enable == 0){
		if(alsps_context_obj->is_als_active_data == false)
			ret = cxt->als_ctl.enable_nodata(enable);
		ret = cxt->ps_ctl.enable_nodata(enable);
...
int alsps_aal_get_data()
{
	...
	if(alsps_context_obj->als_data.get_data == NULL){
		ALSPS_ERR("aal:ALS get_data not exsit\n");
		return -1;
	}
	if(alsps_context_obj->ps_data.get_data == NULL){
		ALSPS_ERR("aal:PS get_data not exsit\n");
		return -1;
	}
	
	cxt = alsps_context_obj;
	ret = cxt->als_data.get_data(&value,&status);
	ALSPS_ERR("alsps_aal_get_data---als_data.get_dat-- value=%d\n",value);

	ret = cxt->ps_data.get_data(&value,&status);
	ALSPS_ERR("alsps_aal_get_data --ps_data.get_data--value=%d\n",value);
	if(ret < 0)
		return -1;
