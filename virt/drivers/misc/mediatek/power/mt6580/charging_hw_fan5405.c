...
static void hw_bc11_dump_register(void)
{
	...
	kal_uint32 reg_num = PMIC_RG_BC11_BB_CTRL;
	kal_uint32 i = 0;

	for(i=reg_num ; i<=PMIC_RG_BC11_BIAS_EN ; i++)
	{
 ...
 static void hw_bc11_init(void)
 {
	 ...
	 bc11_set_register_value(PMIC_RG_BC11_BIAS_EN, 0x1);
	 ...
	 bc11_set_register_value(PMIC_RG_BC11_RST, 0x1);
	 //BC11_BB_CTRL=1
	 //upmu_set_rg_bc11_bb_ctrl(0x1);
	 bc11_set_register_value(PMIC_RG_BC11_BB_CTRL, 0x1);
 ...
 static U32 hw_bc11_stepC1(void)
 {
	...
	//RG_BC11_IPU_EN[1.0] = 01
	//upmu_set_rg_bc11_ipu_en(0x1);
	bc11_set_register_value(PMIC_RG_BC11_IPU_EN, 0x1);
	//RG_BC11_VREF_VTH = [1:0]=10
	//upmu_set_rg_bc11_vref_vth(0x2);
	bc11_set_register_value(PMIC_RG_BC11_VREF_VTH, 0x2);
	//RG_BC11_CMP_EN[1.0] = 01
	//upmu_set_rg_bc11_cmp_en(0x1);
	bc11_set_register_value(PMIC_RG_BC11_CMP_EN, 0x1);
...
//add by yangbo
extern kal_uint8 fan5405_STAT_flag;
 ...
 static kal_uint32 charging_hw_init(void *data)
 {
...
#if 1  //whether 0 gyg wait for test 2015.3.9
...
#endif
	battery_log(BAT_LOG_CRTI, "gpio_number=0x%x,gpio_on_mode=%d,gpio_off_mode=%d\n", gpio_number, gpio_on_mode, gpio_off_mode);
	...
	pmic_set_register_value(PMIC_RG_USBDL_RST, 0x1);
		...
		battery_log(BAT_LOG_CRTI,"high voltage gyg");
		fan5405_reg_config_interface(0x06,0x49);// ISAFE = 1050mA, VSAFE = 4.38V
	#else
	fan5405_reg_config_interface(0x06,0x70);
	#endif

	battery_log(BAT_LOG_CRTI,"fan5405_STAT_flag=%d\n",fan5405_STAT_flag);
	if (1 == fan5405_STAT_flag){	    
	fan5405_reg_config_interface(0x00,0xC0);	//kick chip watch dog
	}else{
	fan5405_reg_config_interface(0x00,0x80);	//add by yangbo disable red
	}

	fan5405_reg_config_interface(0x01,0xb8);	//TE=1, CE=0, HZ_MODE=0, OPA_MODE=0
	fan5405_reg_config_interface(0x05,0x04);  //modify by yangbo for Vsp=4.533V
	if ( !charging_init_flag ) {   
		fan5405_reg_config_interface(0x04,0x19); //146mA
		charging_init_flag = KAL_TRUE;
	}      

	//battery_log(BAT_LOG_CRTI,"[fan5405] finish config,  dump register begin-gyg: \n");
 ...
 static kal_uint32 charging_enable(void *data)
 {
	...
	else
	{
		...
		#if 1  //whether 0 wait for test gyg 2015.3.9
