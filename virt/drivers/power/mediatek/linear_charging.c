...
void set_vol_according_to_temperature(void)
{
	check_battery_temperature();
		...
		switch(g_tempStatus)
               {
		      case E_BAT_T_L_0:
                                   	  g_cv_voltage = BATTERY_VOLT_04_350000_V;
                               break;
                      case E_BAT_T_B_0_10:
                                          g_cv_voltage = BATTERY_VOLT_04_350000_V;
                               break;
                      ...
                      case E_BAT_T_B_48_55:
                                  	 g_cv_voltage = BATTERY_VOLT_04_100000_V;
                               break;
		      case E_BAT_T_U_55:
                                   	 g_cv_voltage = BATTERY_VOLT_04_100000_V;
                               break;
...
void pchr_turn_on_charging(void)
{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		if (g_temp_CC_value == CHARGE_CURRENT_0_00_MA) {
			...
			battery_log(BAT_LOG_CRTI, "set_vol_according_to_temperature**g_cv_voltage=%d***gyg--BATTERY_TEMP_CHARGING_CURRENT_ok\n",g_cv_voltage);
  		#else		
			g_cv_voltage = BATTERY_VOLT_04_350000_V;
		#endif
	#else
